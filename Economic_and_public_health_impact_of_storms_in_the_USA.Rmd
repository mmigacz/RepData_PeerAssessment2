...
```{r echo = FALSE}
data_nrow <- nrow(data)
unique_evtype <- length(unique(data$EVTYPE))
```
The data source contains **`r data_nrow`** events, categorized into **`r unique_evtype`** distinct types.
...
The real number of distinct event types is much lower than **`r unique_evtype`**. Inspection of the reported event types shows numerous inconsistencies, such as using adjectives, shortcuts and synonyms of the same name. Therefore we prepared a set of rules to remove ambiguities and reduce the set of event types.
...
The above rules reduce number of distinct event types to **`r unique_evtype`**. This data cleansing is far from complete and would lead to potential errors in the ranking of event types with few occurrences. However, we applied corrections to the most often occured event types and now their quality seems to be acceptable to answer the questions.
